...
<body>
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            header('location:test.php?q=quiz&step=2&eid=' . $_GET[eid] . '&n=' . $_GET[n] . '&t=' . $_GET[t]);
        ...
        $time = time();
        $q = mysqli_query($conn, "INSERT INTO history VALUES(NULL,'".$_SESSION['username']."','$_GET[eid]' ,'0','0','0','0',NOW(),'$time','ongoing','false')") or die('Error137');
        $_SESSION['6e447159425d2d'] = "6e447159425d2d";
        header('location:test.php?q=quiz&step=2&eid=' . $_GET["eid"] . '&n=' . $_GET["n"] . '&t=' . $_GET["t"]);
    }
}
...
function end(){
  data = prompt("Are you sure to end this Quiz? Remember, once finished, you wont be able to continue. If you want to continue then enter \\"yes\\" in the textbox below and press enter here");
  if(data=="yes"){
    window.location ="test.php?q=quiz&step=2&eid=' . $_GET["eid"] . '&n=' . $_GET["n"] . '&t=' . isset($_GET["total"]) . '&endquiz=end";
  }
}
function enable(){
...
function frmreset(){
  ...
  document.getElementById("qform").reset();
    ...
    if (seconds <= 0) {
        ...
        document.getElementById(\'countdown\').innerHTML = "Time Up !.Stop Now..";
        window.location ="test.php?q=quiz&step=2&eid=' . $_GET["eid"] . '&n=' . $_GET["n"] . '&t=' . isset($_GET["total"]) . '&endquiz=end";
    } else {    
        seconds--;
    }
...
var countdownTimer = setInterval(\'secondPassed()\', 1000);
            ...
            echo '<font size="5" style="margin-left:150px;font-family:\'typo\' font-size:20px; font-weight:bold;color:darkred">Time Left : </font><span class="timer btn btn-default" style="margin-left:20px;"><font style="font-family:\'typo\';font-size:20px;font-weight:bold;color:darkblue" id="countdown"></font></span><span class="timer btn btn-warning" style="margin-left:50px" onclick="end()"><span class=" glyphicon glyphicon-off"></span>&nbsp;&nbsp;<font style="font-size:12px;font-weight:bold">Finish Quiz</font></span>';
            $eid   = @$_GET['eid'];
            $sn    = @$_GET['n'];
            $total = @$_GET['t'];
...
<br />';
            $q = mysqli_query($conn, "SELECT * FROM user_answer WHERE qid='$qid' AND username='".$_SESSION['username']."' AND eid='$_GET[eid]'") or die("Error222");
            if (mysqli_num_rows($q) > 0) {
                $row = mysqli_fetch_array($q);
                $ans = $row['ans'];
                $q = mysqli_query($conn, "SELECT * FROM options WHERE qid='$qid' AND optionid='$ans'") or die("Error222");
                $row = mysqli_fetch_array($q);
                $ans = $row['option'];
            } else {
                $ans = "";
            ...
            if (strlen($ans) > 0) {
                echo "<font style=\"color:green;font-size:12px;font-weight:bold\">Selected answer: </font><font style=\"color:#565252;font-size:12px;\">" . $ans . "</font>&nbsp;&nbsp;<a href=update.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total&qid=$qid&delanswer=delanswer><span class=\"glyphicon glyphicon-remove\" style=\"font-size:16px;color:darkred\"></span></a><br /><br />";
            }
            ...
            while ($row = mysqli_fetch_array($q)) {
                ...
                echo '<div class="funkyradio-success"><input type="radio" id="' . $optionid . '" name="ans" value="' . $optionid . '" onclick="enable()"> <label for="' . $optionid . '" style="width:50%"><div style="color:black;font-size:18px;word-wrap:break-word">&nbsp;&nbsp;' . $option . '</div></label></div>';
            }
            echo '</div>';
            if ($_GET["t"] > $_GET["n"] && $_GET["n"] != 1) {
                echo '<br /><a href="test.php?q=quiz&step=2&eid=' . $eid . '&n=' . ($sn - 1) . '&t=' . $total . '" class="btn btn-primary" style="height:30px">
                <span class="glyphicon glyphicon-arrow-left" aria-hidden="true"  style="font-size:16px;color:white;"></span></a>&nbsp;&nbsp;&nbsp;&nbsp;

                <button type="submit" class="btn btn-default" disabled="true" id="sbutton" style="height:30px"><span class="glyphicon glyphicon-lock" style="font-size:12px" aria-hidden="true"></span><font style="font-size:12px;font-weight:bold"> Lock</font></button>&nbsp;&nbsp;&nbsp;&nbsp;
                ...
                <a href="test.php?q=quiz&step=2&eid=' . $eid . '&n=' . ($sn + 1) . '&t=' . $total . '" class="btn btn-primary" style="height:30px"><span class="glyphicon glyphicon-arrow-right" aria-hidden="true"  style="font-size:16px;color:white;"></span></a>
                </form>
                <br>
                <br>';
            } else if ($_GET["t"] == $_GET["n"]) {
                echo '<br />
                <a href="test.php?q=quiz&step=2&eid=' . $eid . '&n=' . ($sn - 1) . '&t=' . $total . '" class="btn btn-primary" style="height:30px"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"  
                style="font-size:16px;color:white;"></span></a>&nbsp;&nbsp;&nbsp;&nbsp;
                ...
                <a href="test.php?q=quiz&step=2&eid=' . $eid . '&n=' . ($sn + 1) . '&t=' . $total . '" class="btn btn-primary" style="height:30px"><span class="glyphicon glyphicon-arrow-right" aria-hidden="true"  style="font-size:16px;color:white;"></span></a></form><br><br>';
            } else {
            }
            echo '</div>';
            echo '<div class="panel" style="text-align:center">';
            $q = mysqli_query($conn, "SELECT * FROM questions WHERE eid='$_GET[eid]'") or die("Error222");
            $i = 1;
            while ($row = mysqli_fetch_array($q)) {
                $ques[$row['qid']] = $i;
                $i++;
            }
            $q = mysqli_query($conn, "SELECT * FROM user_answer WHERE eid='$_GET[eid]' AND username='".$_SESSION['username']."'") or die("Error222a");
            $i = 1;
            while ($row = mysqli_fetch_array($q)) {
            ...
            for ($i = 1; $i <= $total; $i++) {
                echo '<a href="test.php?q=quiz&step=2&eid=' . $eid . '&n=' . $i . '&t=' . $total . '"  style="margin:5px;padding:5px;background-color:';
                if ($quesans[$i]) {
                    echo "darkgreen";
                } else {
                    echo "darkred";
...
if (@$_GET['q'] == 'result' && @$_GET['eid']) {
    $eid = @$_GET['eid'];
    $q = mysqli_query($conn, "SELECT * FROM test_quiz WHERE eid='$eid' ") or die('Error157');
    while ($row = mysqli_fetch_array($q)) {
        $total = $row['total'];
    }
    $q = mysqli_query($conn, "SELECT * FROM history WHERE eid='$eid' AND username='".$_SESSION['username']."' ") or die('Error157');
...
<center><h1 class="title" style="color:#660033">Result</h1><center><br /><table class="table table-striped title1" style="font-size:20px;font-weight:1000;">';
    ...
    <tr style="color:darkblue"><td style="vertical-align:middle">Score&nbsp;<span class="glyphicon glyphicon-star" aria-hidden="true"></span></td><td style="vertical-align:middle">' . $s . '</td></tr>';
        $q = mysqli_query($conn, "SELECT * FROM rank WHERE  username='".$_SESSION['username']."' ") or die('Error157');
        while ($row = mysqli_fetch_array($q)) {
            $s = $row['score'];
